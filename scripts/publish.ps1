# Quick publish script for shine-rs
param(
    [string]$Version = "",
    [switch]$DryRun = $false
)

Write-Host "🚀 Publishing shine-rs to crates.io" -ForegroundColor Green

# Check if version is provided for non-dry-run
if (-not $DryRun -and $Version -eq "") {
    Write-Host "❌ Please provide version number: .\scripts\publish.ps1 -Version '0.1.0'" -ForegroundColor Red
    exit 1
}
...
if ($DryRun) {
    ...
    # Commit and tag if version provided
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Push to git
        Write-Host "⬆️  Pushing to git..." -ForegroundColor Yellow
