...
use hound;
use shine_rs::mp3_encoder::{encode_pcm_to_mp3, Mp3Encoder, Mp3EncoderConfig, StereoMode};
use std::fs::File;
use std::io::Write;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Generated {} bytes of MP3 data (streaming)",
        streaming_mp3_data.len()
    );
    ...
    println!(
        "   Mono (22kHz, 64kbps): {} bytes -> output_mono.mp3",
        mono_mp3.len()
    );
    ...
    println!(
        "   HQ (48kHz, 320kbps): {} bytes -> output_hq.mp3",
        hq_mp3.len()
    );
