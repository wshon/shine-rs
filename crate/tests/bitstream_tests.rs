...
mod tests {
    ...
    fn test_shine_side_info_structure() {
        ...
        assert_eq!(
            side_info.scfsi[0].len(),
            4,
            "Should have 4 SCFSI bands per channel"
        );
    ...
    fn test_shine_global_config_structure() {
        ...
        assert_eq!(
            ...
            "Default version should be 3 (MPEG_I)"
        );
        assert_eq!(
            config.mpeg.layer, 1,
            "Default layer should be 1 (LAYER_III)"
        );
        ...
        for gr in 0..2 {
            assert_eq!(
                config.side_info.gr[gr].ch.len(),
                2,
                "Should have 2 channels per granule"
            );
        }
    }
    ...
    fn test_side_info_encoding() {
        ...
        bs.put_bits(0, 1)
            .expect("Should write window_switching_flag"); // window_switching_flag
    ...
    fn test_bitstream_real_data_validation() {
        ...
        assert!(
            F1_WRITTEN_BYTES > 400 && F1_WRITTEN_BYTES < 450,
            "Frame 1 size should be reasonable"
        );
        assert!(
            F2_WRITTEN_BYTES > 400 && F2_WRITTEN_BYTES < 450,
            "Frame 2 size should be reasonable"
        );
        assert!(
            F3_WRITTEN_BYTES > 400 && F3_WRITTEN_BYTES < 450,
            "Frame 3 size should be reasonable"
        );

        // Total size should be reasonable for 3 frames
        let total_bytes = F1_WRITTEN_BYTES + F2_WRITTEN_BYTES + F3_WRITTEN_BYTES;
        assert!(
            total_bytes > 1200 && total_bytes < 1300,
            "Total 3 frames should be reasonable size"
        );

        // Bits per frame should be consistent with 128kbps at 44.1kHz
        let expected_bits = (128000 * 1152) / 44100; // ~3344
        assert!(
            (BITS_PER_FRAME - expected_bits).abs() < 10,
            "Bits per frame should match bitrate calculation"
        );
    }
    ...
    fn test_slot_lag_real_data_validation() {
        ...
        assert!(
            (F2_SLOT_LAG_BEFORE - F1_SLOT_LAG_AFTER).abs() < 0.000001,
            "Slot lag continuity broken between F1 and F2"
        );
        assert!(
            (F3_SLOT_LAG_BEFORE - F2_SLOT_LAG_AFTER).abs() < 0.000001,
            "Slot lag continuity broken between F2 and F3"
        );
        ...
        assert!(
            (f1_diff - 0.040816).abs() < 0.000001,
            "Frame 1 slot lag increment incorrect"
        );
        assert!(
            (f2_diff - 0.040816).abs() < 0.000001,
            "Frame 2 slot lag increment incorrect"
        );
        assert!(
            (f3_diff - 0.040816).abs() < 0.000001,
            "Frame 3 slot lag increment incorrect"
        );
    }
    ...
    fn test_frame_size_calculation() {
        ...
        assert!(
            frame_size_128 >= 416 && frame_size_128 <= 418,
            "128 kbps frame size should be ~417 bytes"
        );
        ...
        assert!(
            frame_size_192 >= 625 && frame_size_192 <= 627,
