...
mod unit_tests {
    ...
    fn test_config_validation_invalid_sample_rate() {
        ...
        assert!(matches!(
            config.validate(),
            Err(ConfigError::UnsupportedSampleRate(12345))
        ));
    }
    ...
    fn test_config_validation_invalid_bitrate() {
        ...
        assert!(matches!(
            config.validate(),
            Err(ConfigError::UnsupportedBitrate(999))
        ));
    }
    ...
    fn test_config_validation_incompatible_combinations() {
        ...
        match config.validate() {
            ...
            other => panic!(
                "Expected IncompatibleRateCombination error, got: {:?}",
                other
            ),
        }
    ...
    fn test_config_validation_valid_combinations() {
        ...
        let config = Mp3EncoderConfig::new().sample_rate(8000).bitrate(32);
        assert!(config.validate().is_ok());

        let config = Mp3EncoderConfig::new().sample_rate(11025).bitrate(64);
        assert!(config.validate().is_ok());

        // MPEG-2 valid combinations
        let config = Mp3EncoderConfig::new().sample_rate(16000).bitrate(80);
        assert!(config.validate().is_ok());

        let config = Mp3EncoderConfig::new().sample_rate(22050).bitrate(160);
        assert!(config.validate().is_ok());

        // MPEG-1 valid combinations
        let config = Mp3EncoderConfig::new().sample_rate(32000).bitrate(32);
        assert!(config.validate().is_ok());

        let config = Mp3EncoderConfig::new().sample_rate(44100).bitrate(320);
        assert!(config.validate().is_ok());
    }
    ...
    fn test_encoder_creation_with_invalid_config() {
        ...
        assert!(matches!(
            ...
            Err(EncoderError::Config(
                ConfigError::IncompatibleRateCombination { .. }
            ))
        ));
    }
...
mod integration_tests {
    ...
    fn test_simple_encoding_stereo() {
        ...
        assert!(
            frames.len() > 0 || !final_data.is_empty(),
    ...
    fn test_simple_encoding_mono() {
        ...
        let mut test_data = Vec::new();
        for i in 0..2304 {
            // 2 frames worth of mono data
            let sample =
                ((i as f32 * 440.0 * 2.0 * std::f32::consts::PI / 22050.0).sin() * 16384.0) as i16;
            test_data.push(sample);
        }
    ...
    fn test_separate_channels_stereo() {
        ...
        for i in 0..2304 {
            // 1 frame worth of stereo data
            let left_sample =
                ((i as f32 * 440.0 * 2.0 * std::f32::consts::PI / 44100.0).sin() * 16384.0) as i16;
            let right_sample =
                ((i as f32 * 880.0 * 2.0 * std::f32::consts::PI / 44100.0).sin() * 16384.0) as i16;
            left_channel.push(left_sample);
            right_channel.push(right_sample);
        }

        let frames = encoder
            .encode_separate_channels(&left_channel, Some(&right_channel))
            .unwrap();
        assert!(!frames.is_empty(), "Should produce encoded frames");
    }
    ...
    fn test_separate_channels_mono() {
        ...
        let frames = encoder
            .encode_separate_channels(&mono_channel, None)
            .unwrap();
        assert!(!frames.is_empty(), "Should produce encoded frames");
    }
    ...
    fn test_streaming_encoding() {
        ...
        for chunk_idx in 0..5 {
            ...
            for i in 0..2304 {
                ...
                let sample = ((sample_idx as f32 * 440.0 * 2.0 * std::f32::consts::PI / 44100.0)
                    .sin()
                    * 16384.0) as i16;
...
mod error_handling_tests {
    ...
    fn test_incompatible_rate_combination_error() {
        ...
        if let Err(EncoderError::Config(ConfigError::IncompatibleRateCombination {
            sample_rate,
            bitrate,
            reason,
        })) = encoder
