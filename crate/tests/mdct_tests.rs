...
mod tests {
    ...
    fn test_mdct_granule_overlap() {
        ...
        assert_eq!(
            MDCT_WINDOW_SIZE, 1152,
            "MDCT window should be twice granule size"
        );
        ...
        assert_eq!(
            MDCT_WINDOW_SIZE / 2,
            GRANULE_SIZE,
            "Window half should equal granule size"
        );
        assert_eq!(
            OVERLAP_SIZE,
            GRANULE_SIZE / 2,
            "Overlap should be half granule size"
        );
    }
    ...
    fn test_mdct_frequency_mapping() {
        ...
        assert_eq!(
            INPUT_SAMPLES, OUTPUT_COEFFS,
            "MDCT should preserve sample count"
        );
        ...
        assert!(
            (FREQ_BIN_WIDTH - 38.28125).abs() < 0.001,
            "Frequency bin width should be ~38.28 Hz"
        );
        ...
        assert!(
            (max_freq - nyquist_freq).abs() < 1.0,
            "MDCT should cover up to Nyquist frequency"
        );
    }
    ...
    fn test_mdct_energy_properties() {
        ...
        assert!(
            energy_per_sample > 0,
            "Average energy per sample should be positive"
        );
    }
    ...
    fn test_mdct_aliasing_reduction() {
        ...
        let test_ca_values = [
            -0.6f64, -0.535, -0.33, -0.185, -0.095, -0.041, -0.0142, -0.0037,
        ];
        ...
        for &ca in &test_ca_values {
            ...
            assert!(
                ...
                "CA² + CS² should approximately equal 1"
            );
        }
    }
}
