...
mod tests {
    ...
    fn test_constants_match_shine() {
        ...
        assert!(
            (PI - std::f64::consts::PI).abs() < 1e-15,
            "PI should match std::f64::consts::PI"
        );
        assert!(
            (SQRT2 - std::f64::consts::SQRT_2).abs() < 1e-15,
            "SQRT2 should match std::f64::consts::SQRT_2"
        );
        assert!(
            (LN2 - std::f64::consts::LN_2).abs() < 1e-15,
            "LN2 should match std::f64::consts::LN_2"
        );
    }
    ...
    fn test_structure_sizes() {
        ...
        log::debug!(
            "ShineGlobalConfig size: {}",
            mem::size_of::<ShineGlobalConfig>()
        );

        // Basic sanity checks - structures shouldn't be empty
        assert!(
            mem::size_of::<GrInfo>() > 0,
            "GrInfo should have non-zero size"
        );
        assert!(
            mem::size_of::<ShineSideInfo>() > 0,
            "ShineSideInfo should have non-zero size"
        );
        assert!(
            mem::size_of::<L3Loop>() > 0,
            "L3Loop should have non-zero size"
        );
        assert!(
            mem::size_of::<ShineGlobalConfig>() > 0,
            "ShineGlobalConfig should have non-zero size"
        );

        // Verify structures aren't unreasonably large
        assert!(
            mem::size_of::<ShineGlobalConfig>() < 1024 * 1024,
            "Config should not be too large"
        );
    }
    ...
    fn test_array_bounds() {
        ...
        assert_eq!(
            config.l3_enc.len(),
            MAX_CHANNELS,
            "L3 enc should have MAX_CHANNELS"
        );
        assert_eq!(
            config.l3_enc[0].len(),
            MAX_GRANULES,
            "L3 enc should have MAX_GRANULES"
        );
        assert_eq!(
            config.l3_enc[0][0].len(),
            GRANULE_SIZE,
            "L3 enc should have GRANULE_SIZE"
        );

        assert_eq!(
            config.mdct_freq.len(),
            MAX_CHANNELS,
            "MDCT freq should have MAX_CHANNELS"
        );
        assert_eq!(
            config.mdct_freq[0].len(),
            MAX_GRANULES,
            "MDCT freq should have MAX_GRANULES"
        );
        assert_eq!(
            config.mdct_freq[0][0].len(),
            GRANULE_SIZE,
            "MDCT freq should have GRANULE_SIZE"
        );

        assert_eq!(
            config.scalefactor.l.len(),
            MAX_GRANULES,
            "Scalefactor should have MAX_GRANULES"
        );
        assert_eq!(
            config.scalefactor.l[0].len(),
            MAX_CHANNELS,
            "Scalefactor should have MAX_CHANNELS"
        );
        assert_eq!(
            config.scalefactor.l[0][0].len(),
            22,
            "Scalefactor should have 22 bands"
        );
    }
    ...
    fn test_mp3_standard_compliance() {
        ...
        assert_eq!(
            GRANULE_SIZE, 576,
            "MP3 Layer III requires 576 samples per granule"
        );
        ...
        assert!(
            gr_info.big_values <= (GRANULE_SIZE / 2) as u32,
