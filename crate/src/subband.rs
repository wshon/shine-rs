...
pub fn shine_subband_initialise(subband: &mut Subband) {
    ...
    (0..SBLIMIT).rev().for_each(|i| {
        // matches shine: for (i = SBLIMIT; i--;)
        (0..64).rev().for_each(|j| {
            // matches shine: for (j = 64; j--;)
...
pub fn shine_window_filter_subband(
    ...
    for i in (0..32).rev() {
        // i from 31 down to 0 (matches shine: for (i = 32; i--;))
        if ptr_offset < buffer.len() {
            subband.x[ch][i + subband.off[ch] as usize] = (buffer[ptr_offset] as i32) << 16;
        }
        ptr_offset += stride;
    }
    ...
    for i in (0..SBLIMIT).rev() {
        // i from SBLIMIT-1 down to 0 (matches shine: for (i = SBLIMIT; i--;))
        let mut s_value: i32;
