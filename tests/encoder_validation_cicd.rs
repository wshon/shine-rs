...
fn load_reference_manifest() -> HashMap<String, ReferenceConfig> {
    ...
    let manifest_content =
        fs::read_to_string(manifest_path).expect("Failed to read reference manifest");

    let manifest: serde_json::Value =
        serde_json::from_str(&manifest_content).expect("Failed to parse reference manifest JSON");

    let reference_files = manifest["reference_files"]
        .as_object()
...
fn test_standard_configurations() {
    ...
    println!(
        "\nStandard configurations test summary: {} passed, {} failed",
        passed, failed
    );
...
fn test_reference_file_integrity() {
    ...
    if !corrupted_files.is_empty() {
        ...
        println!(
            "   Run 'python scripts/generate_reference_validation_data.py' to regenerate them."
        );
    }
