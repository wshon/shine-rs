...
fn test_different_configurations() {
    ...
    for (sample_rate, channels, bitrate) in &test_configs {
        ...
        match shine_initialise(&config) {
            Ok(mut encoder) => {
                println!(
                    "✅ Config {}Hz {}ch {}kbps: initialization successful",
                ...
                match unsafe { shine_encode_buffer_interleaved(&mut encoder, dummy_data.as_ptr()) }
                {
                    Ok((_, written)) => {
                        println!(
                            "✅ Config {}Hz {}ch {}kbps: encoding successful ({} bytes)",
                            sample_rate, channels, bitrate, written
                        );
                    }
                    Err(e) => panic!(
                        "❌ Config {}Hz {}ch {}kbps: encoding failed: {}",
            ...
            Err(e) => panic!(
                "❌ Config {}Hz {}ch {}kbps: initialization failed: {}",
                sample_rate, channels, bitrate, e
            ),
        }
    }
...
fn test_multiple_frames() {
    ...
    println!(
        "✅ Multiple frames test: {} total bytes output",
        total_output
    );
