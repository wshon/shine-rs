...
fn main() {
    ...
    // Test each frame independently
    for frame_num in 1..=3 {
        println!("\n=== Testing Frame {} Independently ===", frame_num);
        
        // Create fresh encoder for each frame
        ...
        if end_idx <= pcm_data.len() {
            ...
            if written >= 4 {
                ...
                // Show first 16 bytes
                println!("  First 16 bytes: {:02X?}", &mp3_data[..16.min(written)]);
            }
        }
    }
}