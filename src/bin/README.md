...
ffprobe -v quiet -print_format json -show_format tests/input/sample-12s.wav
```
...
```bash
# 检查原始WAV文件音量
ffmpeg -i tests/input/sample-12s.wav -af "volumedetect" -f null -

# 检查生成的MP3文件音量
ffmpeg -i tests/output/sample-12s-output.mp3 -af "volumedetect" -f null -
```
...
```bash
# 放大MP3音频以验证数据存在
ffmpeg -i tests/output/sample-12s-output.mp3 -af "volume=70dB" tests/output/amplified-sample.wav

# 检查放大后的音量
ffmpeg -i tests/output/amplified-sample.wav -af "volumedetect" -f null -
```

### 十六进制分析
```bash
# 检查生成的MP3文件内部结构
cargo run --bin mp3_hexdump tests/output/sample-12s-output.mp3

# 对比shine参考实现
cargo run --bin mp3_hexdump tests/input/sample-12s-shine.mp3
```
...
ffmpeg -i tests/output/sample-12s-output.mp3 -af "volumedetect" -f null - 2>&1